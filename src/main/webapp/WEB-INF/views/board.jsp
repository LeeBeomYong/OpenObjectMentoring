...
<%-- 		      <button id="writeBtn" class="btn-write" onclick="location.href='<c:url value="/board/write"/>'"><i class="fa fa-pencil"></i> 글쓰기</button> --%>
        ...
        <br><br><br>
        <c:if test="${null ne session_id }">
        	<a href="write_ok"><button class="btn btn-primary" >글쓰기</button></a><br><br><br>
        </c:if>
        <c:if test="${null eq session_id }">
        	<a href="login"><button class="btn btn-primary" >로그인 후 글쓰기</button></a><br><br><br>
        </c:if>
	...
	<%--         	</c:if> --%>			
				<c:if test="${ph.showPrev}">
	            	<a class="page" href="<c:url value="/board${ph.sc.getQueryString(ph.beginPage-1)}"/>"><span style="font-size: 25px;">◀</span></a>
	          	</c:if>
	<%--         	<c:forEach var="i" begin="${ph.beginPage }" end="${ph.endPage }"> --%>
	<%--         		<a href="<c:url value='/board?page=${i}&pageSize=${ph.pageSize}' />">${i }</a> --%>
	<%--         	</c:forEach> --%>
				<c:forEach var="i" begin="${ph.beginPage}" end="${ph.endPage}">
	            	<a class="page ${i==ph.sc.page? "paging-active" : ""}" href="<c:url value="/board${ph.sc.getQueryString(i)}"/>"><span style="font-size: 25px;">&nbsp;&nbsp;${i}&nbsp;&nbsp;</span></a>
	          	</c:forEach>
	<%--         	<c:if test="${ph.showNext }"> --%>
	<%--         		<a href="<c:url value='/board?page=${ph.endPage+1}&pageSize=${ph.pageSize}' />">▶</a> --%>
	<%--         	</c:if> --%>
				<c:if test="${ph.showNext}">
	            	<a class="page" href="<c:url value="/board${ph.sc.getQueryString(ph.endPage+1)}"/>"><span style="font-size: 25px;">▶</span></a>
	          	</c:if>
	        </div>
        </c:if>
    </div>
