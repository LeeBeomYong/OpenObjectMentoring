    ...
    <div align="center">
        ...
        <div class="mb-3" style = "width: 30%;">
		    <div class="search-container">
		      <form action="<c:url value="admin_boardList"/>" class="search-form" method="get">
...
<%-- 		      <button id="writeBtn" class="btn-write" onclick="location.href='<c:url value="/board/write"/>'"><i class="fa fa-pencil"></i> 글쓰기</button> --%>
        ...
        <table id="table_font" class="table table-hover table-bordered notice-main-table" style=" text-align:center;">
            ...
            <tr>
                ...
                <td>
                	<a href="admin_board_update?no=${dto.getBoard_id() }"><button class="btn btn-success btn-sm">수정</button></a>
                	<a href="admin_board_delete?no=${dto.getBoard_id() }"><button class="btn btn-danger btn-sm">삭제</button></a>
                </td>
            </tr>
                </c:forEach>
            </c:if>
        ...
        <c:if test="${empty List}">
            	<h1>게시글이 없습니다.</h1><br><br>
        </c:if>
        <a href="admin_board_insert"><button class="btn btn-primary btn-sm">게시글 등록</button></a>
	...
	<%--         	</c:if> --%>			
				<c:if test="${ph.showPrev}">
	            	<a class="page" href="<c:url value="/admin_boardList${ph.sc.getQueryString(ph.beginPage-1)}"/>"><span style="font-size: 25px;">◀</span></a>
	          	</c:if>
	<%--         	<c:forEach var="i" begin="${ph.beginPage }" end="${ph.endPage }"> --%>
	<%--         		<a href="<c:url value='/board?page=${i}&pageSize=${ph.pageSize}' />">${i }</a> --%>
	<%--         	</c:forEach> --%>
				<c:forEach var="i" begin="${ph.beginPage}" end="${ph.endPage}">
	            	<a class="page ${i==ph.sc.page? "paging-active" : ""}" href="<c:url value="/admin_boardList${ph.sc.getQueryString(i)}"/>"><span style="font-size: 25px;">&nbsp;&nbsp;${i}&nbsp;&nbsp;</span></a>
	          	</c:forEach>
	<%--         	<c:if test="${ph.showNext }"> --%>
	<%--         		<a href="<c:url value='/board?page=${ph.endPage+1}&pageSize=${ph.pageSize}' />">▶</a> --%>
	<%--         	</c:if> --%>
				<c:if test="${ph.showNext}">
	            	<a class="page" href="<c:url value="/admin_boardList${ph.sc.getQueryString(ph.endPage+1)}"/>"><span style="font-size: 25px;">▶</span></a>
	          	</c:if>
	        </div>
        </c:if>
    </div>
